import { Address, PublicClient, Hash, TransactionRequest, PrivateKeyAccount } from 'viem';
import {
...
export async function setupFastWithdrawal(
  ...
  const currentMinimumAssertionPeriod = await client.rollupAdminLogicReadContract({
  ...
  if (currentMinimumAssertionPeriod !== config.minimumAssertionPeriod) {
    const setMinimumPeriodRequest = await client.rollupAdminLogicPrepareTransactionRequest({
  ...
  const fastConfirmerRequest = await setAnyTrustFastConfirmerPrepareTransactionRequest({
  ...
  const receipt = createRollupPrepareTransactionReceipt(
    await parentChainClient.waitForTransactionReceipt({ hash: fastConfirmerTxHash })
  );
