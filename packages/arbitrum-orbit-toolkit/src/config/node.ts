...
export async function prepareNodeConfiguration(
  ...
  if (getParentChainLayer(params.parentChainId) === 1) {
    if (!params.parentChainBeaconRpcUrl) {
      throw new Error('Parent chain beacon RPC URL required for L2 Orbit chains');
    }
    nodeConfigParameters.parentChainBeaconRpcUrl = params.parentChainBeaconRpcUrl;
  }
